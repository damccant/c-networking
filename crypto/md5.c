...
static inline md5_uint32 SWAP(md5_uint32 n)
{
        unsigned int x = 0x76543210;
        char *c = (char*) &x;
        if(*c == 0x10)
                return n; // little endian
        return ((n << 24) | ((n&65280)<<8) | ((n&16711680)>>8) | (n>>24)); // big endian
}
...
{
        md5_process_bytes(inBuf, inLen, mdContext);
}
...
char* MD5printable_from_context(MD5_CTX *mdContext, char *out)
{
        for(int i = 0; i < 16; i++)
        {
                out[i<<1] = hexchars[mdContext->digest[i] >> 4];
                out[(i<<1) + 1] = hexchars[mdContext->digest[i] & 0x0f];
        }
        out[32] = '\0';
        return out;
}
...
{
        return MD5printable_from_context(mdContext, (char*)malloc(33));
}
...
char* MDString_printable(char *inString, size_t inString_len, char* out)
{
        ...
        MD5Init(&mdContext);
        MD5Update(&mdContext, (unsigned char*)inString, inString_len);
        MD5Final(&mdContext);

        return MD5printable_from_context(&mdContext, out);
}

char* MDString_printable_new(char *inString, size_t inString_len)
{
        return MDString_printable(inString, inString_len, (char*)malloc(33));
}
